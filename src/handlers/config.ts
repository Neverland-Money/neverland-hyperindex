...
indexer.contractRegister(
  ...
  async ({ event, context }) => {
    context.chain.PoolAddressesProvider.add(normalizeAddress(event.params.addressesProvider));
  }
);
...
indexer.contractRegister(
  ...
  async ({ event, context }) => {
    ...
    if (contractId === POOL_ID) {
      context.chain.Pool.add(proxyAddress);
    } else if (contractId === POOL_CONFIGURATOR_ID) {
      context.chain.PoolConfigurator.add(proxyAddress);
    }
  }
);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'ACLAdminUpdated' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'ACLManagerUpdated' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
    await recordProtocolTransaction(context, event.transaction.hash, timestamp);
    const providerId = normalizeAddress(event.srcAddress);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'AddressSet' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
    await recordProtocolTransaction(context, event.transaction.hash, timestamp);
    const providerId = normalizeAddress(event.srcAddress);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'AddressSetAsProxy' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
    await recordProtocolTransaction(context, event.transaction.hash, timestamp);
    const providerId = normalizeAddress(event.srcAddress);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'MarketIdSet' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
    await recordProtocolTransaction(context, event.transaction.hash, timestamp);
    const providerId = normalizeAddress(event.srcAddress);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'OwnershipTransferred' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
    await recordProtocolTransaction(context, event.transaction.hash, timestamp);
    const providerId = normalizeAddress(event.srcAddress);
...
indexer.onEvent(
  { contract: 'PoolAddressesProvider', event: 'PriceOracleSentinelUpdated' },
  async ({ event, context }) => {
    const timestamp = Number(event.block.timestamp);
    await recordProtocolTransaction(context, event.transaction.hash, timestamp);
    const providerId = normalizeAddress(event.srcAddress);
...
indexer.contractRegister(
  ...
  async ({ event, context }) => {
    context.chain.AToken.add(normalizeAddress(event.params.aToken));
    context.chain.VariableDebtToken.add(normalizeAddress(event.params.variableDebtToken));
    if (normalizeAddress(event.params.stableDebtToken) !== ZERO_ADDRESS) {
      context.chain.StableDebtToken.add(normalizeAddress(event.params.stableDebtToken));
    }
  }
);
...
indexer.contractRegister(
  { contract: 'UserVaultFactory', event: 'UserVaultCreated' },
  async ({ event, context }) => {
    context.chain.UserVault.add(normalizeAddress(event.params.vault));
  }
);
...
indexer.onEvent({ contract: 'UserVault', event: 'LoanSelfRepaid' }, async ({ event, context }) => {
  await recordProtocolTransaction(
    context,
    event.transaction.hash,
    Number(event.block.timestamp),
