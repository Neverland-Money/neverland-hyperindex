...
test('settlements do not sync LP positions from chain when env flags are enabled', async () => {
  ...
  try {
    ...
    const userLeaderboardState = createStore<UserLeaderboardState>();
    const userReserveList = createStore<UserReserveList>();
    ...
    const lpPoolState = createStore<LPPoolState>();
    const lpPoolPositionIndex = createStore<LPPoolPositionIndex>();
...
test('settlements baseline uses zero checked block when no change and block missing', async () => {
  ...
  try {
    const leaderboardState = createStore<LeaderboardState>();
    const leaderboardEpoch = createStore<LeaderboardEpoch>();
    const userNFTBaseline = createStore<UserNFTBaseline>();
    const userNFTOwnership = createStore<UserNFTOwnership>();
    const userLeaderboardState = createStore<UserLeaderboardState>();
    const userEpochStats = createStore<UserEpochStats>();
    const userReserveList = createStore<UserReserveList>();
    const userTokenList = createStore<UserTokenList>();
